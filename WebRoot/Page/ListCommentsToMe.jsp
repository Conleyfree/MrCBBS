...
<body>
    <%	List<Comment> comments = (List<Comment>)request.getAttribute("commentToMe");
        List<String> postTitle = (List<String>)request.getAttribute("postTitle");
        User user = (User)request.getSession().getAttribute("User");
        if(user == null)	response.sendRedirect("../login.jsp");
    %>
    <div class="alert alert-info">当前位置<b class="tip"></b>评论我的</div>
    <table class="tbform list">
        ...
        <%	if(comments.size()==0){	%>
            <tr>
                <td><a href="WritePo.jsp">还没有人评论过你的呢！QAQ</a></td>
            </tr>
        <%
            }else{
            ...
            <tr>
                <td>
                    <%  String s = comments.get(i).getCContent();
                        if(s.length() < 10){     %>
                    <%=s %>
                    <%  }else{     %>
                    <%=s.subSequence(0,10) %> . . . . . .
                    <%  }  %>
                    <a style="font-size: large;color: #0b9cd3" href="getPoAction.action?PID=<%=comments.get(i).getPID() %>">
                        @<%=postTitle.get(i) %>
                    </a>
                </td>
            </tr>
